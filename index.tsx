...
const restaurantSelectorMobile = document.getElementById('restaurant-selector-mobile') as HTMLSelectElement;
...
const calculateHourlyDemand = (restaurant, date, hora) => {
    ...
    // FIX: Add types to reduce function arguments to prevent TS error with unknown[] from Object.values
    const sum_F_hora = Object.values(hourlyFactorsForDay).reduce((a: number, b: number) => a + b, 0);
...
const updateUIText = () => {
    ...
    // FIX: Cast iterated element to HTMLElement to access dataset property
    ...
    // FIX: Cast iterated element to HTMLElement to access dataset property
     document.querySelectorAll<HTMLElement>('[data-lang-key-placeholder]').forEach(el => {
        ...
        // FIX: Cast element to HTMLInputElement to access placeholder property
...
languageSelector.addEventListener('change', (e) => {
    ...
    state.currentLanguage = (e.target as HTMLSelectElement).value;
    updateUIText();
});

const updateRestaurantControls = (newRestaurant) => {
    // FIX: Cast iterated element to HTMLElement to access dataset property
    document.querySelectorAll<HTMLElement>('#restaurant-tabs .tab').forEach(tab => {
        tab.classList.toggle('active', tab.dataset.restaurant === newRestaurant);
    });
    // FIX: Cast restaurantSelectorMobile to HTMLSelectElement to access value property
    (restaurantSelectorMobile as HTMLSelectElement).value = newRestaurant;
};
...
restaurantTabs.addEventListener('click', e => {
    // FIX: Cast event target to HTMLElement to access closest method
    const tab = (e.target as HTMLElement).closest('.tab');
    if (tab && (tab as HTMLElement).dataset.restaurant) {
        handleRestaurantChange((tab as HTMLElement).dataset.restaurant);
    }
});

restaurantSelectorMobile.addEventListener('change', (e) => {
    // FIX: Cast event target to HTMLSelectElement to access value property
    handleRestaurantChange((e.target as HTMLSelectElement).value);
});
...
document.addEventListener('click', (e) => {
    // FIX: Cast event target to HTMLElement to access closest method
    const closeTrigger = (e.target as HTMLElement).closest<HTMLElement>('.modal-close, .modal-footer .btn-secondary[data-modal-id]');
    if (closeTrigger) {
...
const openStaffEditModal = (area, employeeId = null) => {
    // FIX: Cast form to HTMLFormElement to access reset method
    ...
    (document.getElementById('staff-edit-id') as HTMLInputElement).value = employeeId || '';
    (document.getElementById('staff-edit-area') as HTMLInputElement).value = area;
    ...
    if (employeeId) {
        ...
        (document.getElementById('staff-edit-nombre') as HTMLInputElement).value = employee.nombre;
        (document.getElementById('staff-edit-rol') as HTMLInputElement).value = employee.rol;
        (document.getElementById('staff-edit-contrato') as HTMLInputElement).value = String(employee.contratoSem);
    } else {
...
document.getElementById('staff-edit-save').addEventListener('click', () => {
    ...
    const id = (document.getElementById('staff-edit-id') as HTMLInputElement).value;
    const area = (document.getElementById('staff-edit-area') as HTMLInputElement).value;
    const restaurant = state.currentRestaurant;
    // FIX: Define employeeData type to allow for an optional 'id' property
    const employeeData: { nombre: string; rol: string; contratoSem: number; id?: string } = {
        nombre: (document.getElementById('staff-edit-nombre') as HTMLInputElement).value.trim(),
        rol: (document.getElementById('staff-edit-rol') as HTMLInputElement).value.trim(),
        contratoSem: parseInt((document.getElementById('staff-edit-contrato') as HTMLInputElement).value, 10)
    };
    ...
    if (id) {
        ...
        // FIX: Add id property to employeeData. This is now allowed due to the type definition above.
        employeeData.id = 'E' + Date.now();
        state.dotacion[restaurant][area].push(employeeData as any);
...
document.getElementById('assignment-grid').addEventListener('click', e => {
    // FIX: Cast event target to HTMLElement to access closest method
    const cell = (e.target as HTMLElement).closest('.assign-cell');
    if (!cell) return;
...
document.getElementById('assignment-save-btn').addEventListener('click', () => {
    ...
    const restaurant = state.currentRestaurant;
    const area = (document.getElementById('assignment-grid') as HTMLElement).dataset.area;
    ...
    // FIX: Cast iterated element to HTMLElement to access dataset property
    document.querySelectorAll<HTMLElement>('#assignment-grid .assign-cell.is-assigned').forEach(cell => {
...
const exportEmployeeSchedulePDF = (employeeId) => {
    ...
    // FIX: Cast window to any to access jspdf property
    ...
    doc.text(`${t('weekOf')} ${weekStart.getDate()} ${t('to')} ${weekEnd.getDate()} ${t('de')} ${monthName} ${weekStart.getFullYear()}`, 14, 30);
...
const absenceForm = document.getElementById('absence-form') as HTMLFormElement;
const absenceDaySelect = document.getElementById('absence-day') as HTMLSelectElement;
const absenceReasonSelect = document.getElementById('absence-reason') as HTMLSelectElement;
...
const populateAbsenceSelects = () => {
    ...
    getCurrentWeekDates().forEach((date, i) => {
        ...
        // FIX: No error, but ensuring consistency. `add` is valid on HTMLSelectElement
...
const openAbsenceModal = (employeeId) => {
    ...
    // FIX: Cast element to HTMLInputElement to access value property
    (document.getElementById('absence-employee-id') as HTMLInputElement).value = employeeId;
    // FIX: No error, but ensuring consistency. `reset` is valid on HTMLFormElement
    absenceForm.reset();
...
absenceForm.addEventListener('submit', (e) => {
    ...
    const employeeId = (document.getElementById('absence-employee-id') as HTMLInputElement).value;
    const employee = getEmployeeById(employeeId);
    const date = (document.getElementById('absence-day') as HTMLSelectElement).value;
    const motivo = (document.getElementById('absence-reason') as HTMLSelectElement).value;
    const notas = (document.getElementById('absence-notes') as HTMLTextAreaElement).value.trim();
...
document.getElementById('absence-list').addEventListener('click', e => {
    // FIX: Cast event target to HTMLElement to access closest method
    const deleteButton = (e.target as HTMLElement).closest<HTMLButtonElement>('button[data-action="delete-absence"]');
    if (deleteButton) {
        // FIX: Cast element to HTMLInputElement to access value property
        const employeeId = (document.getElementById('absence-employee-id') as HTMLInputElement).value;
...
const populateLogFilters = () => {
    const employeeFilter = document.getElementById('log-filter-employee') as HTMLSelectElement;
    const typeFilter = document.getElementById('log-filter-type') as HTMLSelectElement;
...
const renderLogbook = () => {
    ...
    // FIX: Cast elements to access value property
    const employeeFilter = (document.getElementById('log-filter-employee') as HTMLSelectElement).value;
    const typeFilter = (document.getElementById('log-filter-type') as HTMLSelectElement).value;
    const startDateFilter = (document.getElementById('log-filter-start-date') as HTMLInputElement).value;
    const endDateFilter = (document.getElementById('log-filter-end-date') as HTMLInputElement).value;
...
document.getElementById('log-clear-filters-btn').addEventListener('click', () => {
    // FIX: Cast elements to set value property
    (document.getElementById('log-filter-employee') as HTMLSelectElement).value = '';
    (document.getElementById('log-filter-type') as HTMLSelectElement).value = '';
    (document.getElementById('log-filter-start-date') as HTMLInputElement).value = '';
    (document.getElementById('log-filter-end-date') as HTMLInputElement).value = '';
    renderLogbook();
});
...
const toggleAdminMode = () => {
    ...
    // FIX: Convert boolean to string for localStorage
    localStorage.setItem('scheduleAppAdmin', String(state.isAdmin));
    updateAdminControls();
...
const init = () => {
    ...
    // FIX: Cast languageSelector to HTMLSelectElement to access value property
    (languageSelector as HTMLSelectElement).value = state.currentLanguage;
